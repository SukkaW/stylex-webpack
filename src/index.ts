...
import { FUCK_NEXTJS_CHUNK_NAME, INCLUDE_REGEXP, PLUGIN_NAME, STYLEX_CHUNK_NAME, VIRTUAL_ENTRYPOINT_CSS_PATH, VIRTUAL_ENTRYPOINT_CSS_PATTERN, VIRTUAL_FUCK_NEXTJS_CSS_PATTERN } from './constants';
...
export class StyleXPlugin {
  ...
  apply(compiler: webpack.Compiler) {
    ...
    if (this.loaderOption.nextjsMode) {
      compiler.options.optimization.splitChunks.cacheGroups[FUCK_NEXTJS_CHUNK_NAME] = {
        name: FUCK_NEXTJS_CHUNK_NAME,
        test: VIRTUAL_FUCK_NEXTJS_CSS_PATTERN,
        type: 'css/mini-extract',
        chunks: 'all',
        enforce: true
      };
    ...
    compiler.hooks.make.tap(PLUGIN_NAME, (compilation) => {
      NormalModule.getCompilationHooks(compilation).loader.tap(
        ...
        (loaderContext, mod) => {
          ...
          if (VIRTUAL_FUCK_NEXTJS_CSS_PATTERN.test(mod.matchResource || mod.resource)) {
            mod.loaders.push({
          ...
          } else if (INCLUDE_REGEXP.test(extname)) {
      ...
      compilation.hooks.processAssets.tapPromise(
        ...
        async (assets) => {
          ...
          if (this.loaderOption.nextjsMode) {
            ...
            const fuckNextjsChunk = compilation.namedChunks.get(FUCK_NEXTJS_CHUNK_NAME);
